...
def pay():
    ...
    if request.form:
        ...
        payment = client.order.create({'amount' : target_amount, 'currency' : 'INR', 'payment_capture' : '1'})
        event_details = [name_of_event]
        return render_template('navigation.html',event_details=event_details,payment=payment)
    return render_template('navigation.html',event_details=event_details,payment=payment, segment="navigation")
...
def notify_alert():
    notification.notify(
        title = "Please have your meal.",
        message ="Have a light meal to avoid drowsiness. After having the meal take water after 10 minutes.",
        app_icon = r"C:\Users\Jay Munjapara\OneDrive\Desktop\Synchronize.it\SYNChronizeIT\app\base\static\assets\img\bell.ico",
        timeout= 5
...
def notify_alert_1():
    notification.notify(
        ...
        message ="Time to get hydrated. Please drink atleast 2 glasses of water.",
        app_icon = r"C:\Users\Jay Munjapara\OneDrive\Desktop\Synchronize.it\SYNChronizeIT\app\base\static\assets\img\bell.ico",
        timeout= 5
    )
...
def notifications():
    ...
    notify_alert_1()
    notify_alert()
    return render_template('notifications.html', syptoms_lst=syptoms_lst, segment='notifications')

@blueprint.route('/jetlag')
@login_required
def jetlag():
...
@blueprint.route('/jetlag_form')
def jetlag_form():
    with open('Airport_list - Sheet1.csv', 'r', encoding="utf8") as file:
        reader = csv.reader(file)
        readers = list(reader)
        return render_template('jetlag_form.html',reader = readers)

@blueprint.route('/diagnose')
def diagnose():
    return render_template('diagnosis.html', syptoms_lst=syptoms_lst, segment='diagnosis')

@blueprint.route('/schdl_create',methods=['GET', 'POST'])
def schdl_create():
    if request.form:
        ...
        return render_template('jetlag_schd.html', schedule = schedule, schedule_departure = schedule_departure, from_place = from_place[1], to_place = to_place[1])
    with open('Airport_list - Sheet1.csv', 'r', encoding="utf8") as file:
        reader = csv.reader(file)
        readers = list(reader)
        return render_template('jetlag_form.html',reader = readers)
...
def give_disease():
    ...
    loaded_model = pickle.load(open('xg1.sav', 'rb'))
